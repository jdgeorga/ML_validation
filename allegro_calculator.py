...
from ase_allegro_interface import NeuqIPMultiLayerCalculator


class AllegroCalculator:
    def __init__(self, xyz_file,
                 intralayer_model_paths,
                 interlayer_model_path,
                 device='cuda'):
        """
        Initializes the AllegroCalculator with a specified computing device, 
        atomic structure, and model paths.
    ...
    def calculate_forces_and_energy(self):
        """
        Calculates forces and total energy for the 
        internally stored atomic structure.
        """
        if self.atoms is None:
            raise ValueError("Atom structure not loaded. Load XYZ file first.")
        ...
        self.intralayer_forces = self.atoms.calc.results["forces_intralayer"]
        ...
        self.intralayer_energy = self.atoms.calc.results["energy_intralayer"]
